BEGIN { 
  ...
  $0 ~ /listVolumes/ {listVolumes+=1}
  $0 ~ /listZones/ {listZones+=1}
  $0 ~ /listSecurityGroups/ {listSecurityGroups+=1}
  $0 ~ /listTemplates/ {listTemplates+=1}
  $0 ~ /listDiskOffering/ {listDiskOffering+=1}
  $0 ~ /listSnapshots/ {listSnapshots+=1}
  $0 ~ /listVirtualMachines/ {listVirtualMachines+=1}
  $0 ~ /listNetworkOfferings/ {listNetworkOfferings+=1}
  $0 ~ /listAccounts/ {listAccounts+=1}

  $0 ~ /createVolume/ {createVolume+=1}
  $0 ~ /createTemplate/ {createTemplate+=1}
  $0 ~ /createSnapshot/ {createSnapshot+=1}
  $0 ~ /deployVirtualMachine/ {deployVirtualMachine+=1}
  $0 ~ /startVirtualMachine/ {startVirtualMachine+=1}

  $0 ~ /destroyVirtualMachine/ {destroyVirtualMachine+=1}
  $0 ~ /deleteVolume/ {deleteVolume+=1}
  $0 ~ /deleteTemplate/ {deleteTemplate+=1}


END { 

  print "\n"
  print "CloudStack API Summary for" , substr(FILENAME,16,10);
  print "==========================================\n" ;
